...
Exercises the PyO3 bindings (PyTimeFamily, PyCryptoServer, PyFortis) to ensure
...
from fortias_p2p import PyTimeFamily, PyFortis, PyCryptoServer
...
class TestCrossValidation:
    ...
    def test_stamp_produces_identical_hash(self):
        ...
        The content_hash embedded in the Fortis stamp MUST be the SHA-256 of the
        ...
        assert fortis.content_hash == expected_hash, (
            f"PyTimeFamily hash {fortis.content_hash.hex()} != "
    ...
    def test_stamp_verify_roundtrip(self):
        ...
        against the returned Fortis. Must return True. Verifying against
        ...
        fortis = tf.stamp(content)
        assert tf.verify(content, fortis) is True
        assert tf.verify(b"different content", fortis) is False

    def test_fortis_json_roundtrip(self):
        """Stamp, serialize to JSON, deserialize with from_json, verify still valid.

        Tests that PyFortis.to_json() and PyFortis.from_json() are lossless
        ...
        # Deserialize back to PyFortis
        fortis_restored = PyFortis.from_json(json_str)

        # Verification must still succeed after roundtrip
        assert tf.verify(content, fortis_restored) is True

    def test_multi_tick_integrity(self):
        """Create 10 ticks, verify all stamps from tick 0-9 are valid.

        Stamps 10 messages, collects all Fortis records, then verifies each
        ...
        for content in content_list:
            fortis = tf.stamp(content)
            fortis_list.append((content, fortis))

        # Verify tick numbers are sequential
        for i, (_, fortis) in enumerate(fortis_list):
            assert fortis.tick_number == i + 1, (
                f"Expected tick {i + 1}, got {fortis.tick_number}"
            )

        # Verify all stamps
        for content, fortis in fortis_list:
            assert tf.verify(content, fortis) is True, (
                f"Verification failed for tick {fortis.tick_number}"
            )
