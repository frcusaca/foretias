...
fn main() {
    ...
    Command::new("cmake")
        .args([
            "--build",
            liboqs_build_dir.to_str().unwrap(),
            "--config",
            "Release",
            "--parallel",
    ...
    let sources = [
        ...
        "src/signing_p256.c",
        "src/signing_sphincs.c",
        "src/signing_dilithium.c",
        "src/signing_tbid.c",
        "src/kem_mlkem.c",
        "src/hash_sha256.c",
        "src/hash_blake3.c",
    ...
    println!(
        "cargo:rerun-if-changed={}/include/foretias_core.h",
        core_dir.display()
    );
    println!(
        "cargo:rerun-if-changed={}",
        core_dir.join("src/signing_tbid.c").display()
    );
    ...
    bindings
        .write_to_file(out.join("core_bindings.rs"))
        .unwrap();
    ...
    let out_lines: Vec<String> = src.lines().map(|s| s.to_string()).collect();
    ...
    // Strip Copy/Clone from ForetiasPrivKey32 (secret key material must not be copied)
    let final_src = final_src.replace(
        "#[derive(Copy, Clone)]\npub struct ForetiasPrivKey32 {",
        "pub struct ForetiasPrivKey32 {",
    );

    // Prepend allow attributes for FFI-imposed naming conventions
    let final_src = format!(
