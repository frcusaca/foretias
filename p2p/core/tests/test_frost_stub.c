...
static void test_frost_round1_unsupported(void) {
    FortiasFrostRound1 state;
    FortiasResult r = fortias_frost_round1(&state);
    ASSERT_EQ(r, FORTIAS_ERR_UNSUPPORTED, "round1 returns UNSUPPORTED");
}

static void test_frost_sign_share_unsupported(void) {
    FortiasFrostRound1 state;
    FortiasFrostShare key_share, sig_share;
    ...
    FortiasResult r = fortias_frost_sign_share(&state, &key_share, msg, 4, commits, 1, 0, &sig_share);
    ASSERT_EQ(r, FORTIAS_ERR_UNSUPPORTED, "sign_share returns UNSUPPORTED");
}

static void test_frost_aggregate_unsupported(void) {
    FortiasFrostShare share;
    int32_t indices[] = {0};
    const uint8_t commits[64] = {0};
    const uint8_t msg[] = "test";
    FortiasSig64 sig;
    FortiasResult r = fortias_frost_aggregate(&share, indices, 1, commits, msg, 4, &sig);
    ASSERT_EQ(r, FORTIAS_ERR_UNSUPPORTED, "aggregate returns UNSUPPORTED");
}
...
static void test_frost_destroy_no_crash(void) {
    ...
    memset(&state, 0, sizeof(state));
    /* Should not crash */
    fortias_frost_destroy_round1(&state);
