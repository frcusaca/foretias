...
static void test_rng_nonzero_output(void) {
    uint8_t buf[32];
    FortiasResult r = fortias_rng_bytes(buf, sizeof(buf));
    ASSERT_EQ(r, FORTIAS_OK, "rng returns OK");
    int nonzero = 0;
...
static void test_rng_different_calls(void) {
    uint8_t a[32], b[32];
    fortias_rng_bytes(a, sizeof(a));
    fortias_rng_bytes(b, sizeof(b));
...
static void test_rng_exact_length(void) {
    ...
    FortiasResult r = fortias_rng_bytes(buf, 64);
    ASSERT_EQ(r, FORTIAS_OK, "rng fills exact length");
...
static void test_rng_null_zero_len_ok(void) {
    FortiasResult r = fortias_rng_bytes(NULL, 0);
    ASSERT_EQ(r, FORTIAS_OK, "NULL buf with 0 len returns OK");
}

static void test_rng_null_nonzero_len_error(void) {
    FortiasResult r = fortias_rng_bytes(NULL, 32);
    ASSERT_EQ(r, FORTIAS_ERR_BAD_INPUT, "NULL buf with nonzero len returns BAD_INPUT");
}
