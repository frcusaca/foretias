...
pub async fn noise_json_rpc(
    ...
    let result = tokio::time::timeout(timeout, async {
        ...
        let (_pub_key, priv_key) =
            generate_ed25519_keypair().map_err(|e| PtPError::Connect(e.to_string()))?;
        ...
        AsyncReadExt::read_exact(&mut reader, &mut len_buf)
            .await
            .map_err(|e| PtPError::Connect(e.to_string()))?;
        let resp_len = u32::from_le_bytes(len_buf) as usize;
        let mut resp_buf = vec![0u8; resp_len];
        AsyncReadExt::read_exact(&mut reader, &mut resp_buf)
            .await
            .map_err(|e| PtPError::Connect(e.to_string()))?;
        ...
        let response: serde_json::Value =
            serde_json::from_slice(&plaintext).map_err(|e| PtPError::Decode(e.to_string()))?;

        validate_jsonrpc_response(&response).map_err(PtPError::Decode)?;
...
pub async fn noise_json_rpc_typed<T: serde::de::DeserializeOwned>(
    ...
    serde_json::from_value(result).map_err(|e| PtPError::Decode(e.to_string()))
}
...
fn validate_jsonrpc_response(response: &serde_json::Value) -> Result<(), String> {
    ...
    match response.get("jsonrpc").and_then(|v| v.as_str()) {
        ...
        Some(other) => {
            return Err(format!(
                "invalid JSON-RPC response: unexpected version {other:?}"
            ))
        }
