...
/// real ForetisRecord signatures) while still bounded against memory exhaustion.
...
pub(crate) fn process_request_from_value(
    ...
    match method {
        ...
        "get_chronon" => Ok(handlers::handle_get_chronon(server, params)),
        "get_chronon_chain" => Ok(handlers::handle_get_chronon_chain(server, params)),
