...
use fortias_core::crypto_server::CryptoServer;
use fortias_core::error::NodeError;
use fortias_core::fortias::types::{PublicKey, Tbid};
use libp2p::PeerId;
use rand::Rng;
use std::sync::Arc;
...
impl TbidHandshake {
    ...
    pub async fn create_proof(
        ...
        let public_key = match self.crypto.public_key() {
            fortias_core::crypto_server::PublicKeyBytes::Ed25519(pk) => pk.bytes,
    ...
    pub async fn verify_proof(
        ...
        let valid = self.crypto.verify_ed25519(
            &fortias_core::core::bindings::FortiasPubKey32 { bytes: response.public_key },
            &response.signed_payload,
            &fortias_core::core::bindings::FortiasSig64 { bytes: response.signature },
        ).unwrap_or(false);
...
mod tests {
    use super::*;
    use fortias_core::crypto_server::{new_software, FortiasCurve};

    fn test_crypto() -> Arc<dyn CryptoServer> {
        Arc::from(
            new_software(FortiasCurve::Ed25519).expect("create software crypto"),
        )
    }
