...
mod tests {
    ...
    impl PeerTransport for DummyTransport {
        ...
        async fn get_calendar_slice(
            ...
            _tick_start: u64,
            _count: u64,
        ) -> Result<Vec<fortias_core::fortias::TickRecord>, crate::communerd::transport::TransportError> {
            Ok(Vec::new())
        }
    ...
    fn peer_pool_add_and_get() {
        ...
        rt.block_on(async {
            ...
            pool.add_peer(PeerAddr { json_rpc: "127.0.0.1:4001".into() }).await;
    ...
    fn peer_pool_remove() {
        ...
        rt.block_on(async {
            let pool = PeerPool::new(
                vec![PeerAddr { json_rpc: "127.0.0.1:4001".into() }],
                Arc::new(DummyTransport),
                30,
            );
            pool.remove_peer(&PeerAddr { json_rpc: "127.0.0.1:4001".into() }).await;
            let peers = pool.get_peers().await;
