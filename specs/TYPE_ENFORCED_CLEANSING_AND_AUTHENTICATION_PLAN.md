...
**Status:** ✅ MERGED to alpha (Phases A-F). Superseded by `COMBINED_GROUP1_TYPE_BASED_SAFETY_ENFORCEMENT_TAKE_3_PLAN.md`. Checkboxes marked 2026-06-02.
...
- [x] Chose `RANDOM` differentiator for the worktree path: `48291`
      (2026-05-21 10:45)
- [x] `export FULL_WORKTREE_PATH=${HOME}/tmp/foretias-worktrees/TYPE_ENFORCED_CLEANSING_AND_AUTHENTICATION_48291`
- [x] `export BRANCH_NAME=type-enforced-cleansing-and-auth-48291`
...
- [x] `export CARGO_TARGET_DIR="${HOME}/.cache/cargo/foretias-type-enforced-ca-48291"` (per-worktree target dir per `AGENTS.md`)
...
- [x] **A.1** Create `p2p/core-engine/src/foretias/clean_auth.rs`:
...
- [x] **A.2** Register the module in `p2p/core-engine/src/foretias/mod.rs`:
  ```rust
  pub mod unverified;
  ```

- [x] **A.3** Build and verify compilation:
  ```bash
  cd ${FULL_WORKTREE_PATH}/p2p && cargo build -p foretias-core
  ```

- [x] **A.4** Add unit tests in `p2p/core-engine/src/foretias/clean_auth.rs` (inline `#[cfg(test)]`):
...
- [x] **B.1.1** Refactor `handle_ship_ack`:
...
- [x] **B.1.2** Refactor `handle_stream_tick`:
...
- [x] **B.1.3** Refactor `handle_verify` (local path):
...
- [x] **B.1.4** Refactor `cross_node_verify`:
...
- [x] **B.1.5** Update imports:
  ```rust
  use foretias_core::foretias::unverified::*;
  ```

- [x] **B.1.6** Update existing tests to use the new handler signatures.

- [x] **B.1.7** Add new tests in `foretias-server/tests/handler_verification.rs` (NEW):
...
- [x] **B.2.1** Replace `verify_report_signature` stub:
...
- [x] **B.2.2** Refactor `handle_gossip_message`:
...
- [x] **B.2.3** Add reporter key resolution logic:
...
- [x] **B.2.4** Add tests in `foretias-server/tests/probity_verification.rs` (NEW):
...
- [x] **B.3.1** Add `signature` field to `PeerRegistrationRecord`:
  ```rust
...
- [x] **B.3.2** Refactor `gossip_event_loop` (DHT branch):
...
- [x] **B.3.3** Refactor `stamp_peer`:
  - JSON → `UnprocessedForetis` → verify against local calendar → return.

- [x] **B.3.4** Refactor `route_stamp`:
  - Same pattern as `stamp_peer`.

- [x] **B.3.5** Refactor `get_calendar_slice` (remote):
  - Raw → `Vec<UnprocessedChrononRecord>` → chain verify → return.

- [x] **B.3.6** Add tests in `foretias-server/tests/dht_verification.rs` (NEW):
...
- [x] **B.4.1** Update `tick.rs` `verify_pair`:
...
- [x] **C.1** Update `chronomatter/mod.rs` `verify`:
  ```rust
  pub async fn verify(
      &self,
...
- [x] **C.2** Update any callers of `Chronomatter::verify` to pass `UnprocessedForetis`.
...
- [x] **D.1** Update `calendar_store/encrypted_jsonl.rs`:
...
- [x] **D.2** Update `server/handlers.rs` `get_calendar_slice` (local path):
  - Return `Vec<ExternalizedChrononRecord>` for wire transmission.
  - (Remote path already uses Unprocessed in Phase B.3.)

- [x] **D.3** Update `communerd/p2p/rpc_protocol.rs`:
  - Serialize `Externalized<X>` types for libp2p RPC responses.

- [x] **D.4** Add tests in `core-engine/tests/externalize_roundtrip.rs` (NEW):
...
- [x] **E.1** Add a compile-time gate: attempt to compile code that passes an `Unprocessed<X>` directly to a function expecting `CleanAuthenticated<X>`. This should fail to compile. If it compiles, the gate has failed.
...
- [x] **E.2** Add a grep-based CI gate (extend `.github/workflows/secret-discipline.yml` or create `unverified-discipline.yml`):
  ```bash
...
- [x] **E.3** Add integration test in `foretias-server/tests/e2e_verification.rs` (NEW):
