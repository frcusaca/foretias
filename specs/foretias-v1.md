...
Foretias makes backdating impossible not by policy, but by cryptographic construction. A Foretias timestamp (a **Foretis**) binds content to a chronon number in a time being's calendar. Each chronon has its own Ed25519 keypair. When a time being advances from chronon n to chronon n+1, it **destroys the private key for chronon n**. After destruction, no valid signature for chronon n can ever be produced again.

The guarantee is simple:

> `Foretis(content)` always returns a chronon number >= any previously issued chronon. It is structurally impossible to produce a Foretis that appears to be from the past.
...
- **Genetic features** (set at creation, never change):
  ...
  - `chronon_ns` (float): fixed chronon interval in nanoseconds.
  - `serialized` (bool): whether the time being computes sparse chronons or only when it stamps.
- **Epigenetic information** (changes over time, mutex-protected):
  - `chronon` (uint64): current chronon counter, counting nanoseconds since Unix epoch.
  - `calendar` (list[ChrononRecord]): append-only log of all chronons.
- **Runtime secret** (in-memory only):
  - The current private key (Ed25519).

**Lifecycle:**

1. **Created** with `tbid`, `tbn`, `chronon`, and `serialized`. Generates its first keypair for chronon 0. The calendar is initialized with one record: `chronon_number` = 0, `public_key` = the new public key, `forward_foretis` and `backward_foretis` are computed from an existing chrononchain auto-attestation (auto-attestation(genesis, genesis)).
2. **Advances chronons**:
   - If `serialized = False`: a background daemon thread calls `chronon()` every `chronon_ns` nanoseconds. `stamp()` simply signs under the current chronon.
   - If `serialized = True`: no background thread. `chronon()` advances only when `stamp()` is called, and only one caller is permitted at a time (mutex-protected).
3. **Stamps content** under the current chronon's private key.
...
- A time being never produces two different signatures for the same `(chronon, content_hash)` pair.
...
- Public keys can only attest to Foretises signed after their own start boundary. The existence of a subsequent chronon defines the end of the period during which a Foretis could have taken place.

### 2.2 Chronon: the Foretias Self-Attestation Methodology

A **chronon** is a discrete duration of time in the Foretias protocol. Each chronon has its own Ed25519 keypair. Chronons form a numbered chain: chronon 0, chronon 1, chronon 2, ...

Chronons are numbered from genesis; each `chronon_number` represents nanoseconds past the Unix epoch. The `chronon_ns` is adhered to for non-serialized time beings at best effort — system load and clock synchronization may cause slight drift.  Nanoseconds are a practical convenience for v1 (fits in a float with sub-nanosecond jitter); not a fundamental limit of the protocol.

**The chronon: Foretias Auto-Attestation:**

A chronon occurs between two chronon. For clarity sake, Let's call the chronon `OLD`, and `NEW`. `OLD` has duration preceeds NEW entirely.
For the old chronon, we have it's begining `OLD.chronon_number`, we also have `OLD.private_key`, `OLD.secrete_key`:

1. Determine and store `NEW.chronon_number`, roughly the current unix epoch nanoseconds.
2. Generate keypair `(NEW.public_key, NEW.secret_key)`
3. Generate 16-byte RNG nonce (via `foretias_rng_bytes` in C11).
4. Compute the `auto_attestation_blob=concat(TBID, OLD.chronon_number, OLD.public_key, NEW.chronon_number, NEW.public_key, nonce)`.
5. Compute **forward auto-attestation** by signing the `auto_attestation_blob` from the old chronon:
   `forward_foretis = _stamp(content=auto_attestation_blob, tbid=TBID, chronon_number=OLD.chronon_number, private_key=OLD.private_key)`
...
6. Compute **backward auto-attestation** by signing `auto_attestation_blob` with the new private key:
   `backward_foretis = _stamp(content=auto_attestation_blob, tbid=TBID, chronon_number=NEW.chronon_number, private_key=NEW.private_key)`
    This proves the new key acknowledges that it follows the old chronon.
7. Append the new chronon record to the calendar: `ChrononRecord(NEW.chronon_number, NEW.public_key, forward_foretis, backward_foretis, nonce)`.
8. Destroy `old_sk` — it is never held in memory again.
9. The new keypair becomes the active signing key.

**Why this matters:**

- Forward auto-attestation (`forward_foretis`) answers: "Can the new key sign for itself?" — proving the key was actively created at this chronon.
...
**Auto-attestation** is the mechanism a time being uses to continue its own clock. When a chronon advances, the old private key is destroyed and a new keypair generated. The new chronon record contains `forward_foretis` (signed by the old key) and `backward_foretis` (signed by the new key) — both over the same auto-attestation blob. This proves both keys acknowledge the transition.
...
@dataclass
class ChrononRecord:
    chronon_number: uint64      # Nanoseconds since Unix epoch; the chronon's time boundary.
    public_key: bytes        # Ed25519 public key for this chronon.
...
A **calendar** is the append-only log of a time being's chronon chain. It stores every chronon's public key and transition proof in sequential order.

**Operations:**

- `append(chronon_number, public_key, forward_foretis, backward_foretis)` — add a new chronon record.
- `get(chronon_number, count)` — retrieve the earliest `count` chronons at or after `chronon_number`. Returns list[ChrononRecord].
- `latest()` → `int` — the highest chronon number.
...
```json
{
  ...
  "chronons": [
    ...
    {
      "chronon_number": 1697000000000000000,
      "public_key": "hex-encoded Ed25519 public key",
      "forward_foretis": "hex-encoded signature",
      "backward_foretis": "hex-encoded signature"
    }
...
- `chronon_number` is an integer.
- On load, the calendar validates:
  - All `chronon_number` values are non-negative.
  - `chronon_number` values are strictly ascending.
...
A **Chronomatter** (*Chronos authenticus*, the Time Authority) is the active entity that manages ticking an internal clock, and stamping requests. It so by managing Ed25519 key lifecycle, performs stamping and ticking, and publishes chronons to all attached Calendars. It is a standalone class, separate from Calendar storage.

The Chronomatter creates its own genesis chronon record for internal tracking, then publishes an *adapted genesis* per Calendar: Calendar.tbid + Chronomatter's current public key + self-signatures. Note the word can be pronounced /kruh-noh-MAY-ter/.

**Input:**

- `content` (str | bytes): the data to timestamp.
- `chronon_number` (uint64): current chronon, nanoseconds since Unix epoch.
- `public_key` (bytes): active chronon's public key.
- `private_key` (bytes): active chronon's private key.
- `tbid` (bytes): time being's identity.

**Process:**

1. Hash the content: `content_hash = SHA-256(content)`.
2. Concatenate the signature input: `signature_input = concat(tbid, chronon_number, content)`.
3. Sign: `signature = Ed25519_sign(signature_input, private_key)`.
4. Return a `Foretis` containing `(chronon_number, content_hash, signature, tbid, echo, tbn)`.

**Functional implementation:**

```python
def _stamp(content, tbid, chronon_number, public_key, private_key) -> Foretis:
    """Pure function. No side effects. Returns Foretis: `signature = Ed25519_sign(concat(tbid, chronon_number, content), private_key)`."""
    ...
```

### 2.5 Foretis

A **Foretis** (Foretias TimeStamp) is the stamped artifact — the cryptographically signed proof that content existed at a specific chronon.

```python
@dataclass
class Foretis:
    chronon_number: uint64
...
A Foretis is self-contained. To verify it, a verifier needs the Foretis plus access to the time being's calendar (to look up the public key for the claimed chronon).

### 2.6 Time Family (Latin: *Chronos adunatrix*)

The **Time Family** (*Chronos adunatrix*, the messenger) is the orchestrator that coordinates between `Chronomatter` (the Time Authority) and `Calendar` (passive chronon storage). It provides the user-facing API, delegating to the Chronomatter for stamping/ticking and to the Calendar for storage.
...
| **Calendar** | *Chronos graphus* | Passive chronon storage, persistence, integrity checks |
...
- `chronon()` → Chronomatter advances, publishes ChrononRecord to all attached Calendars
- `verify(content, foretis, calendar)` → Chronomatter uses Calendar's chronons for key lookup
- Calendar creates its own genesis via Chronomatter's adapted genesis (Calendar.tbid + Chronomatter's key)

**Verification process:**

1. Extract `chronon_number`, `content_hash`, and `signature` from the Foretis.
2. Look up the public key for `chronon_number` in the calendar using `calendar.get(chronon_number, 1)`.
3. Verify `SHA-256(content) == content_hash`.
4. Verify the Ed25519 signature against the public key.
5. If a `next_chronon_number` is provided, find the exact record matching next_chronon_number from calendar and perform `_verify_pair` on the calendar entries for chronon_number and next_chronon_number.
6. Return results.
...
```python
def verify(content, foretis, next_chronon_number: uint64 | None = None) -> bool | tuple[bool, bool | None]:
    """
    If next_chronon_number is None:
        Returns bool: True if signature and content hash are valid.
    If next_chronon_number is not None:
        Returns (sig_valid, window_closed):
            sig_valid: True if signature and content hash are valid.
            window_closed: True if next chronon exists (key destroyed), False if not (still active).
...
1. **Signature check**: The Ed25519 signature in the Foretis must verify against the public key published for the claimed chronon in the calendar.
2. **Content check**: `SHA-256(provided_content)` must equal the `content_hash` in the Foretis.
3. **Window check** (optional): If a `next_chronon_number` is provided, verify that the chronon's `forward_foretis` is valid — proving a subsequent chronon exists and the current key has been destroyed.
...
- **Pair verification** (`_verify_pair(A, B)`): Given two consecutive chronon records A and B, verify both cross-stamp signatures:
  - `forward_foretis` of B verifies against A's public key.
  - `backward_foretis` of B verifies against B's public key.
- **Chain verification** (`_verify_chain(chronons, return_failures=False)`): Iterates `_verify_pair()` across the entire chain. Returns `(bool, list[int] | None)`. If `return_failures=True`, returns the indexes of failed pairs.
...
foretias/
  ...
  _time.py           # Pure functional: _chronon(), _stamp(), _verify(), _genesis_ma()
  crypto.py          # Ed25519 primitives + SHA-256
  models.py          # ChrononRecord, Foretis dataclasses
...
valid, window_closed = tbf.verify("my message", foretis, next_chronon_number=1)
# valid=True, window_closed=True (chronon 0's key is destroyed)
...
TimeFamily(
    ...
    serialized: bool = False,        # Chronon only advances on stamp
...
|--------|-----------|---------|-------------|
| `stamp` | `(content: str \| bytes) -> Foretis` | Foretis | Sign content under current chronon key. For serialized timebeings, advances chronon before signing. |
| `verify` | `(content, foretis, next_chronon_number=None) -> bool \| (bool, bool \| None)` | bool or tuple | Verify signature, content hash, and optionally window closed. |
| `current_tick` | `() -> int` | int | Current chronon number. |
| `tick` | `() -> None` | None | Advance the calendar to next chronon. For non-serialized: background thread calls this. |
| `get` | `(chronon_number: int, count: int) -> list[ChrononRecord]` | list[ChrononRecord] | Return earliest `count` chronons at or after `chronon_number`. |
...
- Non-serialized timebeings spawn a `threading.Thread` daemon in `__init__` that runs `chronon()` every `chronon_ns` nanoseconds.
- The daemon thread acquires the lock, calls `_chronon()`, mutates state, persists.
...
| Method | Signature | Returns | Description |
|--------|-----------|---------|-------------|
| `stamp` | `(content: str \| bytes) -> Foretis` | Foretis | Sign content under current chronon key. Returns Foretis to caller (not stored). |
| `verify` | `(content, foretis, calendar) -> bool` | bool | Verify signature and content hash using calendar for key lookup. |
| `tick` | `() -> None` | None | Advance to next chronon, publish to all attached Calendars. |
| `get` | `(chronon_number: int, count: int) -> list[ChrononRecord]` | list[ChrononRecord] | Return chronons by Chronomatter's internal counter. |
...
| `serialized` | `bool` | Whether chronon advances only on stamp. |
| `chronon_ns` | `float` | Chronon interval in nanoseconds. |
...
```python
class Calendar:
    chronons: list[ChrononRecord]

    def __init__(self, tbid, tbn, chronons=None, stamp_tbid=None):
        """Create calendar. stamp_tbid defaults to tbid; set when Chronomatter attaches for integrity checks."""
        ...

    def append(self, chronon_record: ChrononRecord) -> None:
        ...

    def get(self, chronon_number: int, count: int) -> list[ChrononRecord]:
        """Return earliest `count` chronons at or after `chronon_number`."""
        ...

    def latest(self) -> int | None:
        """Return the highest chronon number, or None if empty."""
        ...

    def load(path: str) -> Calendar:
        """Load from JSON file. Validates ascending chronon_numbers, hex parse, chain integrity."""
        ...
...
- `_chronon()` returns correct cross-stamp signatures.
...
- `_verify()` with and without `next_chronon_number`.

### 5.2 Calendar Tests (`test_calendar.py`)

- `load()` / `save()` roundtrip with hex encoding.
- `get(chronon_number, count)` returns correct subset.
- `integrity_check()` validates correct chains.
- Invalid JSON (malformed hex, non-ascending chronon_numbers) raises on load.
...
- `serialized=True`: stamp triggers chronon.
- `serialized=False`: stamp does not trigger chronon; background thread does.
- `get(chronon_number, count)` returns correct records.
...
# Create a time being with 1-minute chronons
tbf = TimeFamily(name="alpha", chronon_ns=60_000_000_000.0)
...
# Verify with window check (requires next chronon to exist)
is_valid, window_closed = tbf.verify("hello world", foretis, next_chronon_number=1)
