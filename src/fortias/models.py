...
class TickRecord:
    ...
    Attributes:
        ...
        forward_fortis: Signature of mutual_acknowledgement by this tick's
                        *previous* private key. ``None`` for the genesis record.
        backward_fortis: Signature of mutual_acknowledgement by this tick's
                         private key. Never ``None`` for non-genesis ticks.
    """

    tick_number: int
    public_key: bytes
    forward_fortis: bytes | None
    backward_fortis: bytes

    def __post_init__(self) -> None:
        if self.backward_fortis is None:
            raise TypeError("backward_fortis must never be None for non-genesis ticks")
